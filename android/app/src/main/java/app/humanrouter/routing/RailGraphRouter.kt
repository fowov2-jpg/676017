 ...
 * durations are constrained estimates generated by the runtime builder. Only METRO and MCC are
 * enabled here. MCD and TRAIN are handled by [RailTimetableRouter], so a topology edge can never be
 * mistaken for a published commuter-train departure.
 */
internal class RailGraphRouter private constructor(
        ...
        for ((index, geometricMeters) in candidates) {
            val cost = if (walkGraph != null) {
                if (towardDestination) {
                    walkGraph.shortestWalk(nodes[index].point, point, ACCESS_MAX_SECONDS, ACCESS_MAX_METERS)
                } else {
                    walkGraph.shortestWalk(point, nodes[index].point, ACCESS_MAX_SECONDS, ACCESS_MAX_METERS)
                }
    ...
    private fun serviceAcceptsBoarding(mode: TransportMode, epochSec: Long): Boolean {
        return RailServiceWindow.acceptsBoarding(mode, epochSec)
    }

    private fun serviceBoundaryUncertaintySeconds(mode: TransportMode, epochSec: Long): Int {
        return RailServiceWindow.metroBoundaryUncertaintySeconds(mode, epochSec)
    }
