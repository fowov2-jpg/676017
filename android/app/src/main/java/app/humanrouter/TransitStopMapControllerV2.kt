...
/**
 * Typed transport markers plus a compact stop/station action sheet.
 *
 * This controller is deliberately presentation-only: stop coordinates, transport modes and
 * directions come from the installed routing runtime. It never invents live vehicle positions.
 */
internal object TransitStopMapControllerV2 {
    ...
    private class Controller(private val activity: MainActivity) {
        ...
        private val directions = TransitStopDirectionRepository(activity)
        ...
        private val cameraIdleListener = MapLibreMap.OnCameraIdleListener {
            handler.removeCallbacks(refreshMarkers)
            handler.postDelayed(refreshMarkers, 950L)
        }

        private val panelLayoutListener = View.OnLayoutChangeListener { _, _, _, _, _, _, _, _, _ ->
            handler.removeCallbacks(refreshMarkers)
            handler.postDelayed(refreshMarkers, 80L)
        }

        private val refreshMarkers = Runnable { renderMarkers() }

        init {
            prefs.registerOnSharedPreferenceChangeListener(prefListener)
            nearbyPanel.addOnLayoutChangeListener(panelLayoutListener)
            attach(0L)
            attach(350L)
            attach(1_150L)
        }
        ...
        fun destroy() {
            ...
            nearbyPanel.removeOnLayoutChangeListener(panelLayoutListener)
        ...
        private fun renderMarkers() {
            ...
            val places = if (prefs.getBoolean(AppPreferences.KEY_SHOW_STOPS, true)) {
                nearbyPlaces()
            } else {
                emptyList()
            }
        ...
        private fun loadDirections(place: NearbyTransitPlace, container: LinearLayout) {
            ...
            io.execute {
                val options = if (BuildConfig.DEBUG && placeId.startsWith("qa:")) {
                    ...
                    directions.directionsFor(place, Instant.now().epochSecond, 8)
                }
        ...
        private fun platforms(place: NearbyTransitPlace): List<NearbyTransitPlace> {
            ...
            val grouped = nearbyPlaces().filter { candidate ->
